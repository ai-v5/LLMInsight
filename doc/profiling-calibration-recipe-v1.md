...
- source role 必须与 layout 一致且排序、去重；capture scope 必须与 role-specific 唯一映射完全一致，拒绝 communication-only→matrix、matrix→single/missing、`SINGLE_RANK` 或同 manifest 双 scope 的完整重签；
